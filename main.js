    ...
    try {
      ...
      if (useWebSearch) {
        ...
        if (searchResults.length > 0) {
          ...
          conversationHistory.push({
            role: 'system',
            content: `Web search results for "${input}":\n${webSearchResults}\n\nPlease analyze these results, compare the information from different sources, and use them to provide a comprehensive and accurate answer to the user's query. Highlight any conflicting information and provide a balanced view if there are disagreements between sources. Note that ${searchResults.length > 1 ? 's were' : ' was'} found.`
          });
        } else {
          conversationHistory.push({
            role: 'system',
      ...
      if (model === 'perplexity') {
        try {
          const response = await getChatCompletion(conversationHistory, requestBody);
...
// IPC handler for chat completion
ipcMain.handle('get-chat-completion', async (event, messages) => {
  try {
